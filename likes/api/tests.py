...
class LikeApiTests(TestCase):
    ...
    def test_likes_in_tweets_api(self):
        ...
        self.assertEqual(response.data['tweet']['likes'][0]['user']['id'], self.user1.id)
        self.assertEqual(response.data['tweet']['likes'][1]['user']['id'], self.user2.id)
    ...
    def test_likes_count_in_tweet_with_cache(self):
        tweet = self.create_tweet(self.user1)
        self.create_newsfeed(self.user1, tweet)
        ...
        for i in range(3):
            ...
            self.assertEqual(response.data['tweet']['likes_count'], i + 1)
            tweet.refresh_from_db()
            self.assertEqual(tweet.likes_count, i + 1)

        self.user2_client.post(LIKE_BASE_URL, data)
        response = self.user2_client.get(tweet_url)
        self.assertEqual(response.data['tweet']['likes_count'], 4)
        tweet.refresh_from_db()
        ...
        response = self.user2_client.get(tweet_url)
        self.assertEqual(response.data['tweet']['likes_count'], 3)
    ...
    def test_likes_count_in_comment_with_cache(self):
        ...
        for i in range(3):
            ...
            # check tweet api
            response = client.get(tweet_url)
            self.assertEqual(response.data['tweet']['comments'][0]['likes_count'], i + 1)
            comment.refresh_from_db()
            self.assertEqual(comment.likes_count, i + 1)

        self.user2_client.post(LIKE_BASE_URL, data)
        response = self.user2_client.get(tweet_url)
        self.assertEqual(response.data['tweet']['comments'][0]['likes_count'], 4)
        comment.refresh_from_db()
        ...
        response = self.user2_client.get(tweet_url)
        self.assertEqual(response.data['tweet']['comments'][0]['likes_count'], 3)
