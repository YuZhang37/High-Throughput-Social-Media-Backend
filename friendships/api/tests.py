...
class FriendshipApiTests(TestCase):
    ...
    def test_followers_pagination(self):
        ...
        for i in range(page_size * 2):
            ...
            friendship = self.create_friendship(follower, self.user1)
            friendships.append(friendship)
            if follower.id % 2 == 0:
                self.create_friendship(self.user2, follower)

        url = FOLLOWERS_URL.format(self.user1.id)
        self._test_friendship_pagination(url, 2, friendships)
        ...
        new_followers = [self.create_user('new_follower{}'.format(i)) for i in range(3)]
        new_friendships = []
    ...
    def test_followings_pagination(self):
        ...
        for i in range(page_size * 2):
            ...
            friendship = self.create_friendship(self.user1, following)
            friendships.append(friendship)
            if following.id % 2 == 0:
                self.create_friendship(self.user2, following)
        ...
        new_followings = [self.create_user('new_following{}'.format(i)) for i in range(3)]
        new_friendships = []
    ...
    def _test_friendship_pagination(self, url, expect_pages, friendships):
        results, pages = [], 0
        response = self.anonymous_client.get(url)
