...
from django_hbase.models import RowKeyNotInitializedException, EmptyColumnDataException
...
class HBaseTests(TestCase):
    ...
    def test_filter(self):
        ...
        followings = HBaseFollowing.filter(prefix={'from_user_id': 1})
        ...
        # test limit
        results = HBaseFollowing.filter(prefix={'from_user_id': 1}, limit=1)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].to_user_id, 2)

        results = HBaseFollowing.filter(prefix={'from_user_id': 1}, limit=2)
        self.assertEqual(len(results), 2)
        self.assertEqual(results[0].to_user_id, 2)
        self.assertEqual(results[1].to_user_id, 3)

        results = HBaseFollowing.filter(prefix={'from_user_id': 1}, limit=4)
        self.assertEqual(len(results), 3)
        self.assertEqual(results[0].to_user_id, 2)
        self.assertEqual(results[1].to_user_id, 3)
        self.assertEqual(results[2].to_user_id, 4)

        start = {'from_user_id': 1, 'created_at': results[1].created_at}
        stop = {'from_user_id': 1, 'created_at': results[2].created_at}

        results = HBaseFollowing.filter(start=start, limit=2)
        self.assertEqual(len(results), 2)
        self.assertEqual(results[0].to_user_id, 3)
        self.assertEqual(results[1].to_user_id, 4)

        results = HBaseFollowing.filter(stop=stop, limit=2)
        self.assertEqual(len(results), 2)
        self.assertEqual(results[0].to_user_id, 2)
        self.assertEqual(results[1].to_user_id, 3)

        # test reverse
        results = HBaseFollowing.filter(prefix={'from_user_id': 1}, limit=2, reverse=True)
        self.assertEqual(len(results), 2)
        self.assertEqual(results[0].to_user_id, 4)
        self.assertEqual(results[1].to_user_id, 3)

        start = {'from_user_id': 1, 'created_at': results[1].created_at}
        results = HBaseFollowing.filter(start=start, limit=2, reverse=True)
        self.assertEqual(len(results), 2)
        self.assertEqual(results[0].to_user_id, 3)
